    ...
    >
      <div className="flex max-sm:flex-col items-center xl:gap-10 md:gap-6">
        ...
        {(!isSmallScr || toggleNav) && (
          <ul className="flex items-center text-base xl:gap-1 md:gap-0.5 max-sm:absolute max-sm:flex-col max-sm:gap-4 max-sm:left-4 max-sm:top-20 max-sm:bg-black/75 max-sm:backdrop-blur-sm max-sm:px-2 max-sm:py-4 max-sm:border-2 max-sm:border-cyan-400 max-sm:rounded">
            {NavLinks.map((link) => (
              <Link
                key={link.name}
                href={link.path}
